mapboxgl.accessToken =
  'pk.eyJ1IjoidXJiaWNhIiwiYSI6ImNpbWppN3h3bzAwMWF3aGttdHNuYmtqN2YifQ.EuxNp68ghSVgWPDqokNCPQ';

let start = {
    z: 12.2, center: [-73.991226, 40.740523], bearing: 0, maxZoom: 17, minZoom: 11
  },
  master,
  left,
  right,
...
let currentMode = { id: 'routes', slice: -1 }; // init mode

d3.select('#about-map-button').on('click', () => {
...
master = new mapboxgl.Map({
  ...
  .on('load', (e) => {
    ...
    masterStyle.layers.forEach((l) => {
      if (l.id.substr(0, 2) === 'r_' || l.id.substr(0, 2) === 'l_') l.interactive = true;
    ...
    routesControl.on('click', () => {
      changeMode({
        id: 'routes',
        slice: currentMode.slice,
        feature: currentMode.feature ? currentMode.feature : false
      });
    });
    stationsControl.on('click', () => {
      changeMode({
        id: 'stations',
        slice: currentMode.slice,
        feature: currentMode.feature ? currentMode.feature : false
  ...
  .on('click', (e) => {
    const coords = [[e.point.x - 6, e.point.y - 6], [e.point.x + 6, e.point.y + 6]];
    const features = master.queryRenderedFeatures(coords, { layers: ['s_stations'] });
    if (features.length) {
...
function changeMode(mode) {
  ...
  line_filters = {
    ...
    in_line_2: [
      'all',
      ['>=', slice, 800 * sliceRate * stationRate],
      ['<', slice, 1200 * sliceRate * stationRate]
    ],
    ...
    r_line_2: [
      'all',
      ['>=', slice, 70 * sliceRate * stationRate],
      ['<', slice, 120 * sliceRate * stationRate]
    ],
    r_line_3: ['all', ['>=', slice, 120 * sliceRate * stationRate]]
  };

  stations_filters = {
    av_20: ['all', ['>=', slice_st, 0.1], ['<', slice_st, 0.3]],
  ...
  if (master) {
    if (mode.id !== currentMode.id) {
      routes_layers.forEach((l) => {
        if (mode.id == 'routes') master.setLayoutProperty(l, 'visibility', 'visible');
        else master.setLayoutProperty(l, 'visibility', 'none');
      });
      stations_layers.forEach((l) => {
        if (mode.id == 'stations') master.setLayoutProperty(l, 'visibility', 'visible');
        else master.setLayoutProperty(l, 'visibility', 'none');
      });
    }
...
function getPanel(feature, mode) {
  ...
  d3
    .select('#panel-params-trips-total-value')
    .html(`${feature.properties.outgoing_trips}&nbsp;/&nbsp;${feature.properties.incoming_trips}`);
